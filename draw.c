	...
	while (y < fdf->map->heigth)
	{
		...
		while (x < fdf->map->width)
		{
			if (x < fdf->map->width - 1)
				draw_line(get_points(x, y, fdf, proj), get_points(x + 1, y, fdf, proj), fdf, fdf->map->coords[y][x].z);
			if (y < fdf->map->heigth - 1)
				draw_line(get_points(x, y, fdf, proj), get_points(x, y + 1, fdf, proj), fdf, fdf->map->coords[y][x].z);
			x++;
		}
		y++;
	}
